function boardPaging(pg) { 
    location.href = "/miniSpringWeb/board/boardMain?pg=" + pg; // 페이지 이동
}
...
function hitandView(postElement) {
    ...
    const pg = $("#pg").val();
    const url = "/miniSpringWeb/board/boardView?pg=" + pg + "&seq=" + seq; // URL 생성
    
    // 작성자 아이디와 방문자 아이디가 다를 때만 조회수 증가
    if (authorId !== visitorId) {
        $.ajax({
            type: 'POST',
            url: '/miniSpringWeb/board/increaseHit', // 조회수 증가 URL
            data: { seq: seq },
